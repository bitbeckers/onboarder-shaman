...
contract OnboarderShaman is ReentrancyGuard {
    event YeetReceived(
        address indexed contributorAddress, uint256 amount, address baal, uint256 lootToGive, uint256 lootToPlatform
    );
    ...
    function onboarder20(uint256 _value) public nonReentrant {
        ...
        // amount of loot? fees?
        emit YeetReceived(msg.sender, _value, address(baal), lootToGive, lootToPlatform);
    }
    ...
    function onboarder() public payable nonReentrant {
        ...
        // amount of loot? fees?
        emit YeetReceived(msg.sender, newValue, address(baal), lootToGive, lootToPlatform);
    }
...
contract OnboarderShamanSummoner is CloneFactory, Ownable {
        ...
        returns (address)
    {
        ...
        onboarder.init(_baal, _token, _pricePerUnit, _onlyERC20, _platformFee, _lootPerUnit);

        emit SummonOnbShamanoarderComplete(_baal, address(onboarder), _token, _pricePerUnit, _details, _onlyERC20);
